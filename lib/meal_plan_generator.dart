...
Future<String?> generateMealPlan(List<String> ingredients, List<Item> items) async {
  // Sort items by expiry date or date added (adjust as per your priority)
  
  // Prepare prompt for generative model
  final prompt = 'Generate me a meal that uses $ingredients prioritized by the expiry date or date added to the fridge (Not all items have expirty dates and that is ok.). return this with a title,nutritionalInfo and then recipes there are no dietary requirments. please dont ask any more follow up questions. Return as a Json Do not have ```json or ``` You must allways have the recipe(ingredients, instructions) , calories,fat,carbohydrates,protein. ONLY EVER RETURN ONE THING. CHANGE IT EVERY TIME';
  print(ingredients);
  try {
...
String getApiKey() {
  ...
  return "KEEY"; // Replace with placeholder
}
